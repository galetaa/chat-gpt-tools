...
| Возможность Chromium 6.10.0 | ChatGPT Tools 2.0 |
| --- | --- |
| Открытие selector из popup | Сохранено: `Export Conversation` открывает панель в текущей вкладке |
| Полная обычная беседа | Сохранено: same-origin запрос из isolated world обходит только UI-сокращение, но использует текущую Safari-сессию |
...
Оптимизатор выполняется в `MAIN world` на `document_start` и сокращает только Response, который читает интерфейс ChatGPT. Экспортёр выполняется в изолированном мире Safari. Поэтому его same-origin `fetch` не проходит через подменённый page-world `window.fetch` и получает полную активную ветку. Это устраняет главный риск объединения: экспорт не ограничивается последними 1–100 показанными группами.

Если внутренний endpoint изменён, недоступен или у беседы нет постоянного id, код не ломает страницу и извлекает видимые `user`/`assistant` turns из DOM. В панели явно показано `Full chat` либо `Visible chat`, поэтому fallback не маскируется.
