...
20 августа 2026 года версия 1.1.0 повторно проверена в Safari с реальной авторизованной сессией:

- popup отображается с нативными WebKit switches и полной шириной 360 px;
- стандартная шкала работает от 1 до 20;
- `Extended range` расширяет её до 100, а выключение корректно ограничивает значение до 20;
- настройки сохраняются и переживают повторное открытие popup;
- реальная история разговора сокращается, status indicator показывает результат;
- SPA-переход сбрасывает статус и обновляет его после загрузки без polling;
- ошибок LightSession в консоли не обнаружено.
